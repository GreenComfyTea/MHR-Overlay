...
local monster_UI = {
    ...
    delay = 4,
    delay_percentage_change = 0.001,
    delay_health_change = 20
};
...
function record_health(enemy)
    ...
    if monster_bars[enemy].target_percentage ~= health_percentage then
        monster_bars[enemy].target_percentage = health_percentage;
        monster_bars[enemy].target_health = health;
        monster_bars[enemy].last_changed = get_time()
        if not monster_bars[enemy].current_percentage then
            monster_bars[enemy].current_percentage = health_percentage
            monster_bars[enemy].current_health = health
        end
    end
...
function monster_health()
    ...
    for _, monster in ipairs(monsters) do
        ...
        local monster_bar = monster_bars[monster.enemy];
        ...
        if monster_UI.visibility.health_bar then
            ...
            local health_bar_delayed_health_width = monster_UI.health_bar.width *
                                                        (monster_bar.current_percentage - monster.health_percentage);
            local health_bar_missing_health_width = monster_UI.health_bar.width - health_bar_remaining_health_width -
                                                        health_bar_delayed_health_width;
            ...
            -- remaining health
            draw.filled_rect(screen_position.x + monster_UI.offsets.health_bar.x,
                screen_position.y + monster_UI.offsets.health_bar.y, health_bar_remaining_health_width,
                monster_UI.health_bar.height, remaining_health_color);
            -- delayed health drain
            draw.filled_rect(screen_position.x + monster_UI.offsets.health_bar.x + health_bar_remaining_health_width,
                screen_position.y + monster_UI.offsets.health_bar.y, health_bar_delayed_health_width,
                monster_UI.health_bar.height, monster_UI.colors.health_bar.delayed_health);
            -- missing health
            draw.filled_rect(screen_position.x + monster_UI.offsets.health_bar.x + health_bar_remaining_health_width +
                                 health_bar_delayed_health_width, screen_position.y + monster_UI.offsets.health_bar.y,
                health_bar_missing_health_width, monster_UI.health_bar.height,
                monster_UI.colors.health_bar.missing_health);

            if (get_time() - monster_bar.last_changed) > monster_UI.delay then
                if monster_bar.target_percentage < monster_bar.current_percentage then
                    monster_bar.current_percentage = monster_bar.current_percentage - monster_UI.delay_percentage_change;
                end
                if monster_bar.target_health < monster_bar.current_health then
                    monster_bar.current_health = math.max(monster_bar.current_health - monster_UI.delay_health_change,
                        monster_bar.target_health)
                end
            end
        end

        -- Delayed damage done
        local delayed_damage = monster_bar.current_health - monster_bar.target_health;
        if (delayed_damage > 0) then
            local delayed_damage_text = string.format("%d", delayed_damage);
            draw.text(delayed_damage_text, 5 + screen_position.x + monster_UI.offsets.health_percentage.x +
                monster_UI.shadow_offsets.health_percentage.x, screen_position.y + monster_UI.offsets.monster_name.y +
                monster_UI.shadow_offsets.health_percentage.y, monster_UI.colors.health_percentage.shadow);
            draw.text(delayed_damage_text, 5 + screen_position.x + monster_UI.offsets.health_percentage.x,
                screen_position.y + monster_UI.offsets.monster_name.y, monster_UI.colors.health_percentage.text);
        end

        if monster_UI.visibility.monster_name then
            if monster_UI.shadows.monster_name then
                -- monster name shadow
                draw.text(monster.name, screen_position.x + monster_UI.offsets.monster_name.x +
                    monster_UI.shadow_offsets.monster_name.x, screen_position.y + monster_UI.offsets.monster_name.y +
                    monster_UI.shadow_offsets.monster_name.y, monster_UI.colors.monster_name.shadow);
            end

            -- monster name
            draw.text(monster.name, screen_position.x + monster_UI.offsets.monster_name.x,
                screen_position.y + monster_UI.offsets.monster_name.y, monster_UI.colors.monster_name.text);
        end
        ...
        if monster_UI.visibility.current_health or monster_UI.visibility.max_health then
            ...
            if monster_UI.shadows.health_values then
                ...
                draw.text(health_values, screen_position.x + monster_UI.offsets.health_values.x +
                    monster_UI.shadow_offsets.health_values.x, screen_position.y + monster_UI.offsets.health_values.y +
                    monster_UI.shadow_offsets.health_values.y, monster_UI.colors.health_values.shadow);
            end
            -- health values
            draw.text(health_values, screen_position.x + monster_UI.offsets.health_values.x,
                screen_position.y + monster_UI.offsets.health_values.y, monster_UI.colors.health_values.text);
        end
        ...
        if monster_UI.visibility.health_percentage then
            ...
            if monster_UI.shadows.health_percentage then
                ...
                draw.text(health_percentage_text, screen_position.x + monster_UI.offsets.health_percentage.x +
                    monster_UI.shadow_offsets.health_percentage.x, screen_position.y +
                    monster_UI.offsets.health_percentage.y + monster_UI.shadow_offsets.health_percentage.y,
                    monster_UI.colors.health_percentage.shadow);
            end
            -- health percentage
            draw.text(health_percentage_text, screen_position.x + monster_UI.offsets.health_percentage.x,
                screen_position.y + monster_UI.offsets.health_percentage.y, monster_UI.colors.health_percentage.text);
        end
...
function quest_time()
    ...
    if time_UI.shadow then
        -- shadow
        draw.text(elapsed_time_text, screen_position.x + time_UI.shadow_offset.x,
            screen_position.y + time_UI.shadow_offset.y, time_UI.colors.shadow);
    end
    -- text
...
function damage_meter()
    ...
    for _, player in ipairs(quest_players) do
        ...
        if damage_meter_UI.visibility.damage_bar then
            ...
            elseif top_damage ~= 0 then
                damage_bar_player_damage_width = damage_meter_UI.damage_bar.width *
                                                     (player.display.total_damage / top_damage);
            end
            ...
            -- player damage
            draw.filled_rect(screen_position.x + damage_meter_UI.offsets.damage_bar.x,
                screen_position.y + damage_meter_UI.offsets.damage_bar.y, damage_bar_player_damage_width,
                damage_meter_UI.damage_bar.height, damage_bar_color.player_damage);

            -- other damage
            draw.filled_rect(screen_position.x + damage_meter_UI.offsets.damage_bar.x + damage_bar_player_damage_width,
                screen_position.y + damage_meter_UI.offsets.damage_bar.y, damage_bar_others_damage_width,
                damage_meter_UI.damage_bar.height, damage_bar_color.others_damage);
        end
        ...
        if damage_meter_UI.visibility.id or damage_meter_UI.visibility.name then
            ...
            if damage_meter_UI.shadows.name then
                -- name shadow
                draw.text(name_text,
                    screen_position.x + damage_meter_UI.offsets.name.x + damage_meter_UI.shadow_offsets.name.x,
                    screen_position.y + damage_meter_UI.offsets.name.y + damage_meter_UI.shadow_offsets.name.y,
                    damage_meter_UI.colors.name.shadow);
            end
            -- name
            draw.text(name_text, screen_position.x + damage_meter_UI.offsets.name.x,
                screen_position.y + damage_meter_UI.offsets.name.y, damage_meter_UI.colors.name.text);
        end
        ...
        if damage_meter_UI.visibility.player_damage then
            ...
            if damage_meter_UI.shadows.player_damage then
                ...
                draw.text(player_damage, screen_position.x + damage_meter_UI.offsets.player_damage.x +
                    damage_meter_UI.shadow_offsets.player_damage.x, screen_position.y +
                    damage_meter_UI.offsets.player_damage.y + damage_meter_UI.shadow_offsets.player_damage.y,
                    damage_meter_UI.colors.player_damage.shadow);
            end

            -- player_damage
            draw.text(player_damage, screen_position.x + damage_meter_UI.offsets.player_damage.x,
                screen_position.y + damage_meter_UI.offsets.player_damage.y, damage_meter_UI.colors.player_damage.text);
        end
        ...
        if damage_meter_UI.visibility.player_damage_percentage then
            ...
            if damage_meter_UI.shadows.player_damage_percentage then
                ...
                draw.text(player_damage_percentage_text,
                    screen_position.x + damage_meter_UI.offsets.player_damage_percentage.x +
                        damage_meter_UI.shadow_offsets.player_damage_percentage.x,
                    screen_position.y + damage_meter_UI.offsets.player_damage_percentage.y +
                        damage_meter_UI.shadow_offsets.player_damage_percentage.y,
                    damage_meter_UI.colors.player_damage_percentage.shadow);
            end
            ...
            draw.text(player_damage_percentage_text,
                screen_position.x + damage_meter_UI.offsets.player_damage_percentage.x,
                screen_position.y + damage_meter_UI.offsets.player_damage_percentage.y,
                damage_meter_UI.colors.player_damage_percentage.text);
        end
        i = i + 1;
        ::continue1::
    ...
    if damage_meter_UI.visibility.total_damage then
        ...
        if damage_meter_UI.shadows.total_damage then
            -- total damage shadow
            draw.text(total_damage_text, screen_position.x + damage_meter_UI.offsets.total_damage.x +
                damage_meter_UI.shadow_offsets.total_damage.x, screen_position.y +
                damage_meter_UI.offsets.total_damage.y + damage_meter_UI.shadow_offsets.total_damage.y,
                damage_meter_UI.colors.total_damage.shadow);
        end

        -- total damage
        draw.text(total_damage_text, screen_position.x + damage_meter_UI.offsets.total_damage.x,
            screen_position.y + damage_meter_UI.offsets.total_damage.y, damage_meter_UI.colors.total_damage.text);
    end
end
-----------------------DAMAGE METER UI-----------------------
